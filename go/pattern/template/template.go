...
func (draw *TimeDraw) checkParams(ctx *Context) (err error) {
	fmt.Println(runFuncName(), " TimeDraw  按时间抽奖类型:特殊参数校验...")
	return
}

// getPrizesByNode 获取node奖品信息(不同活动类型实现不同)
func (draw *TimeDraw) getPrizesByNode(ctx *Context) (err error) {
	fmt.Println(runFuncName(), " TimeDraw  do nothing(抽取该场次的奖品即可，无需其他逻辑)...")
	return
}
...
func (draw *TimesDraw) checkParams(ctx *Context) (err error) {
	fmt.Println(runFuncName(), " TimesDraw 按抽奖次数抽奖类型:特殊参数校验...")
	return
}

// getPrizesByNode 获取node奖品信息(不同活动类型实现不同)
func (draw *TimesDraw) getPrizesByNode(ctx *Context) (err error) {
	fmt.Println(runFuncName(), "TimesDraw1. 判断是该用户第几次抽奖...")
	fmt.Println(runFuncName(), "TimesDraw2. 获取对应node的奖品信息...")
	fmt.Println(runFuncName(), "TimesDraw3. 复写原所有奖品信息(抽取该node节点的奖品)...")
	return
}
...
func (draw *AmountDraw) checkParams(ctx *Context) (err error) {
	fmt.Println(runFuncName(), "AmountDraw按数额范围区间抽奖:特殊参数校验...")
	return
}

// getPrizesByNode 获取node奖品信息(不同活动类型实现不同)
func (draw *AmountDraw) getPrizesByNode(ctx *Context) (err error) {
	fmt.Println(runFuncName(), "AmountDraw1. 判断属于哪个数额区间...")
	fmt.Println(runFuncName(), "AmountDraw2. 获取对应node的奖品信息...")
	fmt.Println(runFuncName(), "AmountDraw3. 复写原所有奖品信息(抽取该node节点的奖品)...")
	return
}
